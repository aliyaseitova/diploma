...
function recalc() {
  ...
  const demandReduction = (resTariffEffect + indTariffEffect) * CFG.base_elec;
  ...
  const investCumul = [];
  const BASE_CO2 = CFG.base_co2;
  const BASE_ELEC = CFG.base_elec;
  ...
  for (let year = 2024; year <= 2060; year++) {
    ...
    const nucEffect = year >= s.nucYear
      ? Math.min((year - s.nucYear) / 3, 1) * nucGW * 0.9 * 8760 / 1000 * 0.0
      : 0;
  ...
  const ndc15 = CFG.ndc_unconditional;
  const ndc25 = CFG.ndc_conditional;
  const maxCO2 = 290;
...
loadConfig().then(() => { renderPlants(); recalc(); });